...
Multica can send verification codes and invite emails through either SMTP or [Resend](https://resend.com/).

Provider priority is: **SMTP (`SMTP_HOST`) > Resend (`RESEND_API_KEY`) > stdout fallback**.

| Variable | Default | Description |
|---|---|---|
| `SMTP_HOST` | empty | SMTP server hostname. When set, SMTP is enabled and takes priority |
| `SMTP_PORT` | `587` | SMTP server port |
| `SMTP_USERNAME` | empty | SMTP auth username (optional if relay allows anonymous send) |
| `SMTP_PASSWORD` | empty | SMTP auth password |
| `SMTP_TLS_MODE` | `auto` | TLS mode: `auto`, `starttls`, `ssl`, `none` |
| `SMTP_FROM_EMAIL` | empty | Sender address for SMTP emails. Falls back to `RESEND_FROM_EMAIL`, then `noreply@multica.ai` |
| `SMTP_INSECURE_SKIP_VERIFY` | `false` | Skip TLS cert verification (testing only) |
| `RESEND_API_KEY` | empty | Resend API key (used only when SMTP is not configured) |
| `RESEND_FROM_EMAIL` | `noreply@multica.ai` | Sender address for Resend (must be a domain verified in your Resend account) |
...
| `SMTP_PORT` | `587` | SMTP port. Use `587` for STARTTLS submission, `465` for SMTPS, or `25` for trusted internal plaintext relay |
| `SMTP_USERNAME` | empty | SMTP username. Leave empty for unauthenticated relay |
| `SMTP_PASSWORD` | empty | SMTP password |
| `SMTP_TLS_MODE` | `auto` | Preferred multica-ck TLS mode: `auto`, `starttls`, `ssl`, `none` |
| `SMTP_TLS` | empty | Upstream-compatible alias. `implicit`, `smtps`, and `ssl` force immediate TLS handshake; `starttls` upgrades after connect |
| `SMTP_FROM_EMAIL` | empty | Sender address for SMTP emails. Falls back to `RESEND_FROM_EMAIL`, then `noreply@multica.ai` |
| `SMTP_INSECURE_SKIP_VERIFY` | `false` | Skip TLS cert verification; testing/private CA only |
| `SMTP_TLS_INSECURE` | `false` | Upstream-compatible alias for insecure TLS |
| `SMTP_EHLO_NAME` | machine hostname | EHLO/HELO name announced to the relay. Set a real FQDN when a strict relay rejects the default greeting |

`SMTP_TLS_MODE` and `SMTP_INSECURE_SKIP_VERIFY` take precedence when both local and upstream-compatible names are set. STARTTLS is required when `SMTP_TLS_MODE=starttls`; port `465` auto-enables implicit TLS when mode is unset or `auto`. The dial timeout is 10s and the whole SMTP session has a 30s deadline, so a black-holed relay can't hang the auth handler.

**Behavior when neither SMTP nor Resend is configured**: the server does not error, but every email that should have been sent (verification codes, invite links) **is written to the server's stdout only**. Convenient for local development — copy the code out of the server logs; **in production, forgetting to configure a provider creates a silent black hole**, with users never receiving email and no error surfaced.
