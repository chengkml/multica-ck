...
Multica uses email-based magic link authentication via SMTP or [Resend](https://resend.com).
Provider priority is: `SMTP_HOST` (SMTP) > `RESEND_API_KEY` (Resend) > stdout fallback.

| Variable | Description |
|----------|-------------|
| `SMTP_HOST` | SMTP hostname (enables SMTP provider when set) |
| `SMTP_PORT` | SMTP port (default: `587`) |
| `SMTP_USERNAME` | SMTP auth username (optional for open relay/internal SMTP) |
| `SMTP_PASSWORD` | SMTP auth password |
| `SMTP_TLS_MODE` | SMTP TLS mode: `auto`, `starttls`, `ssl`, `none` |
| `SMTP_FROM_EMAIL` | SMTP sender email address (falls back to `RESEND_FROM_EMAIL`) |
| `SMTP_INSECURE_SKIP_VERIFY` | Skip TLS cert verification (testing only) |
| `RESEND_API_KEY` | Resend API key (used when SMTP is not configured) |
| `RESEND_FROM_EMAIL` | Resend sender email address (default: `noreply@multica.ai`) |
...
Use SMTP when your deployment cannot reach the public internet or you already have an internal mail relay (for example Exchange, Postfix, or an on-prem relay).

| Variable | Description | Default |
|----------|-------------|----------|
| `SMTP_HOST` | SMTP relay hostname; setting this activates SMTP mode | - |
| `SMTP_PORT` | SMTP port | `587` (`465` for implicit TLS) |
| `SMTP_USERNAME` | SMTP username; leave empty for unauthenticated relay | - |
| `SMTP_PASSWORD` | SMTP password | - |
| `SMTP_TLS_MODE` | Preferred TLS mode in multica-ck: `auto`, `starttls`, `ssl`, `none` | `starttls` |
| `SMTP_TLS` | Upstream-compatible alias for TLS mode: `implicit`, `smtps`, `ssl`, `starttls` | - |
| `SMTP_FROM_EMAIL` | SMTP sender email address; falls back to `RESEND_FROM_EMAIL` | - |
| `SMTP_INSECURE_SKIP_VERIFY` | Skip TLS certificate verification for private CA or self-signed certs | `false` |
| `SMTP_TLS_INSECURE` | Upstream-compatible alias for insecure TLS | `false` |
| `SMTP_EHLO_NAME` | EHLO/HELO name announced to the relay. Set a real FQDN when a strict relay rejects the default greeting | machine hostname |

`SMTP_TLS_MODE` and `SMTP_INSECURE_SKIP_VERIFY` take precedence when both local and upstream-compatible names are set. Port 465 auto-enables implicit TLS when the mode is unset or `auto`.
